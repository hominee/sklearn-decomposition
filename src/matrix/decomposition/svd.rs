...
impl<T: Any + Float + Signed> Matrix<T> {
    ...
    unsafe fn golub_kahan_svd_step(
        b: &mut Matrix<T>,
        u: &mut Matrix<T>,
        v: &mut Matrix<T>,
        p: usize,
        ...
        for k in p..n - q - 1 {
...

            {
                ...
                let b_block = MatrixSliceMut::from_matrix(
                    b,
    ...
    fn validate_svd(mat: &Matrix<f64>, b: &Matrix<f64>, u: &Matrix<f64>, v: &Matrix<f64>) {
        ...
        let mut singular_triplets = u_transposed
            ...
            .map(|((u_col, singular_value), v_col)| {
                (
                    Vector::new(u_col.raw_slice()),
                    singular_value,
                    Vector::new(v_col.raw_slice()),
                )
            });

        assert!(singular_triplets
            .by_ref()
