...
use matrix::{BaseMatrix, BaseMatrixMut, Matrix, MatrixSlice, MatrixSliceMut};
...
impl<T: Any + Float> Matrix<T> {
    ...
    pub fn upper_hessenberg(mut self) -> Result<Matrix<T>, Error> {
        ...
        for i in 0..n - 2 {
            let h_holder_vec: Matrix<T>;
            {
                ...
                h_holder_vec =
                    Matrix::make_householder_vec(&lower_slice.iter().cloned().collect::<Vec<_>>())
                        .map_err(|_| {
                            Error::new(
...

            {
                ...
                block -=
                    &h_holder_vec * (h_holder_vec.transpose() * &block) * (T::one() + T::one());
            }

            {
                ...
                block -=
                    (&block * &h_holder_vec) * h_holder_vec.transpose() * (T::one() + T::one());
            }
        }
    ...
    pub fn upper_hess_decomp(self) -> Result<(Matrix<T>, Matrix<T>), Error> {
        ...
        for i in (0..n - 2).rev() {
            ...
            trans_block -=
                &h_holder_vec * (h_holder_vec.transpose() * &trans_block) * (T::one() + T::one());
        }
