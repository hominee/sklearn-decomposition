...
pub trait BaseMatrix<T>: Sized {
    ...
    unsafe fn col_unchecked(&self, index: usize) -> Column<T> {
        ...
        Column {
            col: MatrixSlice::from_raw_parts(ptr, self.rows(), 1, self.row_stride()),
        }
    }
    ...
    unsafe fn row_unchecked(&self, index: usize) -> Row<T> {
        ...
        Row {
            row: MatrixSlice::from_raw_parts(ptr, 1, self.cols(), self.row_stride()),
        }
    }
    ...
    where
        T: Copy + Zero + Add<T, Output = T>,
    {
        self.row_iter().fold(T::zero(), |sum, row| {
            sum + utils::unrolled_sum(row.raw_slice())
        })
    }
        ...
        match axis {
            ...
            Axes::Row => {
                ...
                for row in self.row_iter().skip(1) {
                    utils::in_place_vec_bin_op(&mut mins, row.raw_slice(), |min, &r| {
                        if r < *min {
                            *min = r;
                        }
                    });
        ...
        T: Copy + PartialOrd,
    {
        match axis {
            ...
            Axes::Row => {
                ...
                for row in self.row_iter().skip(1) {
                    utils::in_place_vec_bin_op(&mut maxs, row.raw_slice(), |max, &r| {
                        if r > *max {
                            *max = r;
                        }
                    });
        ...
        T: Copy + Mul<T, Output = T>,
    {
        ...
        for (self_r, m_r) in self.row_iter().zip(m.row_iter()) {
            data.extend_from_slice(&utils::vec_bin_op(
                self_r.raw_slice(),
                m_r.raw_slice(),
                T::mul,
            ));
        ...
        T: Zero + PartialEq,
    {
        ...
        self.iter().enumerate().all(|(i, data)| {
            if i == next_diag {
        ...
        T: Any + Float,
    {
        assert!(
            self.cols() == y.size(),
            "Vector size {0} != {1} Matrix column count.",
            y.size(),
            self.cols()
        );
    ...
    {
        assert!(
            start[0] + rows <= self.rows(),
            "View dimensions exceed matrix dimensions."
        );
        assert!(
            start[1] + cols <= self.cols(),
            "View dimensions exceed matrix dimensions."
        );
        ...
        unsafe {
            MatrixSlice::from_raw_parts(
                self.as_ptr()
                    .offset((start[0] * self.row_stride() + start[1]) as isize),
                rows,
                cols,
                self.row_stride(),
            )
...
pub trait BaseMatrixMut<T>: BaseMatrix<T> {
    ...
    unsafe fn col_unchecked_mut(&mut self, index: usize) -> ColumnMut<T> {
        ...
        ColumnMut {
            col: MatrixSliceMut::from_raw_parts(ptr, self.rows(), 1, self.row_stride()),
        }
    }
    ...
    unsafe fn row_unchecked_mut(&mut self, index: usize) -> RowMut<T> {
        ...
        RowMut {
            row: MatrixSliceMut::from_raw_parts(ptr, 1, self.cols(), self.row_stride()),
        }
    }
    ...
    fn swap_rows(&mut self, a: usize, b: usize) {
        assert!(
            a < self.rows(),
            "Row index {0} larger than row count {1}",
            a,
            self.rows()
        );
        assert!(
        ...
        if a != b {
            unsafe {
                let row_a = slice::from_raw_parts_mut(
                    self.as_mut_ptr().offset((self.row_stride() * a) as isize),
                    self.cols(),
                );
                let row_b = slice::from_raw_parts_mut(
                    self.as_mut_ptr().offset((self.row_stride() * b) as isize),
                    self.cols(),
                );
